...
class PathUtils {
    ...
    public static function getRelativeRoot($current_folder = NULL, $base_folder = ''){
        ...
        $current_folder = preg_replace('/^\/$/', '', $current_folder); // removes '/' from beginning of string

        if(!$current_folder || $current_folder == '/' || $current_folder == './' || !preg_match('/\//', $current_folder)){
            ...
            $relative_root = preg_replace('/[^\/\.]+/', '', $relative_root); // remove non-relative paths
        }
